...
function draw(){
  ...
  if(gameState===0){
    ...
    text("TO MAKE THE NINJA JUMP YOU HAVE TO USE UP ARROW KEY",75,390)
    gameOverFlag = 0
    if(keyDown("S")){
      gameState = 1
    }
  ...
  if(gameState === 2){
    ...
    textSize(12)
    fill("blue")
    text("YAAAY! YOU HAVE FOUND ONE PIECE OF HEART. THERE,STILL 2 PIECES ARE  LEFT",50,200)
  ...
  if(gameState === 4){
    ...
    textSize(12)
    fill("blue")
    text("YAAAY! YOU HAVE FOUND ANOTHER PIECE OF HEART. THERE,STILL 1 PIECE IS  LEFT",50,200)
    text("PLEASE PRESS 'N' TO START THE NEXT LEVEL",150,300)
    if(keyDown("N")){
      gameState = 5
    }
